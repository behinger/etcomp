function result=local_EyelinkTargetModeDisplay(el)
...
while stop==0 && bitand(Eyelink('CurrentMode'), el.IN_TARGET_MODE)
	...
	if (targetvisible==1 && result==0) || tx~=otx || ty~=oty
		local_EyelinkEraseCalTarget(el, targetrect);
		targetvisible = 0;
	end
	% redraw if invisible
...
% 		fprintf( 'Target drawn at: x=%d, y=%d\n', tx, ty );
		...
		targetrect=local_EyelinkDrawCalTarget(el, tx, ty);
		targetvisible = 1;
...
if targetvisible==1
	local_EyelinkEraseCalTarget(el, targetrect);   % erase target on exit, bit superfluous actually
end
local_EyelinkClearCalDisplay(el); % clear_cal_display();
