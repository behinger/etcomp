...
% 		fprintf ('%s\n', 'EyelinkDoTrackerSetup: in ''ImageModeDisplay''' );
    ...
    % a way to escape the script cleanly
    if el.lastKeyCodes(KbName('shift')) &&  el.lastKeyCodes(KbName('escape'))
        error 'Experiment was interrupted by shift + escape'
    end
    find(el.lastKeyCodes);
        
    if key~=0 & key~=el.JUNK_KEY    % print pressed key codes and chars
        fprintf('%d\t%s\n', key, char(key) );
    end
